...
def get_model_path(modelname):
    ...
    pretrained_dir = '/content/drive/MyDrive/SupCon_OCT_Clinical/pretrained_dir'
    PTDICT = {
...
def build_backbone(args):
    ...
    if args.backbone in ['swin_B_224_22k', 'swin_B_384_22k', 'swin_L_224_22k', 'swin_L_384_22k']:
        ...
        if args.pretrained and args.ckpt == '':
            ...
            checkpoint = torch.load(pretrainedpath, map_location=device)['model']
            from collections import OrderedDict
        ...
        if args.pretrained == False and args.ckpt!='':
            print("CHECKING!!!")
            device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
            checkpoint = torch.load(args.ckpt, map_location=device)['model']
            from collections import OrderedDict
            _tmp_st = OrderedDict({k:v for k, v in clean_state_dict(checkpoint).items() if 'head' not in k})
            _tmp_st_output = backbone.load_state_dict(_tmp_st, strict=False)
            print(str(_tmp_st_output))
        ...
        if args.with_transformer_head == True:
            ...
            print("IN SWIN BACKBONE RETURN")
            return backbone
            # shape = (20, 3, 384, 384)
            # random_tensor = torch.rand(*shape)
            # output = backbone(random_tensor)
            # # print(f"ouput shape after del backbone avgpool:{output.shape}")
            # print(f"feature of swin:{output.shape}")
            # exit()
            
    elif args.backbone in ['CvT_w24', 'CvT_21_384']:
        
        backbone = build_CvT(args.backbone, args.num_class)
        if args.pretrained and args.ckpt == '':
            # print("In pretrain = True and ckpt None of CvT")
            pretrainedpath = get_model_path(args.backbone)
            checkpoint = torch.load(pretrainedpath, map_location='cpu')
            from collections import OrderedDict
            _tmp_st = OrderedDict({k:v for k, v in clean_state_dict(checkpoint).items() if 'head' not in k})
            ...
            print(str(_tmp_st_output))
        if args.pretrained == False and args.ckpt!='':
            # print("In pretrain = False and ckpt of CvT")
        ...
        if args.with_transformer_head == True:
            print("CvT with_transformer_head")
            ...
            bb_num_channels = backbone.dim_embed[-1]
            backbone.head = nn.Sequential(
                    nn.Linear(bb_num_channels, bb_num_channels), # num_features of swin_B_384_22k =  1024
                    nn.ReLU(inplace=True),
                    nn.Linear(bb_num_channels, 128)
                )
            print(f"bb_num_channels:{bb_num_channels}")
            backbone.forward = backbone.forward_SupCon
            print("IN CvT BACKBONE RETURN")
            return backbone
